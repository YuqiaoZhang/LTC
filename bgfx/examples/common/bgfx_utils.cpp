...
bgfx::TextureHandle loadTexture(bx::FileReaderI* _reader, const char* _name, uint32_t _flags, uint8_t _skip, bgfx::TextureInfo* _info)
{
	...
	if (NULL != bx::stristr(_name, ".zlib"))
	{
		uint32_t compressedSize;
		void* compressedData = load(filePath, &compressedSize);
		...
		int size;
		char* data = stbi_zlib_decode_malloc((const char*)compressedData, compressedSize, &size);
