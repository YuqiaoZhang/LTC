...
struct Camera
{
    ...
    void setTransform(float* _invViewMtx)
    {
        m_eye[0] = _invViewMtx[12];
        m_eye[1] = _invViewMtx[13];
        m_eye[2] = _invViewMtx[14];

        m_at[0] = _invViewMtx[ 8];
        m_at[1] = _invViewMtx[ 9];
        m_at[2] = _invViewMtx[10];

        m_up[0] = _invViewMtx[4];
        m_up[1] = _invViewMtx[5];
        m_up[2] = _invViewMtx[6];

        if (m_at[2] != 0.0f)
            m_horizontalAngle = -::atanf(m_at[0]/m_at[2]);
        else
            m_horizontalAngle = -1.57079632679f;

        if (m_at[2] != 0.0f)
            m_verticalAngle = -::atanf(m_at[1]/m_at[2]);
        else if(m_at[0] != 0.0f)
            m_verticalAngle = -::atanf(m_at[1]/m_at[0]);
        else
            m_verticalAngle = 1.57079632679f;
    }
...
void cameraSetTransform(float* _invViewMtx)
{
    s_camera->setTransform(_invViewMtx);
}
