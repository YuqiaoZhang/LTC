        ...
        for (uint32_t arrayLayer = 0; arrayLayer < vk_texture_header->arrayLayers; ++arrayLayer)
        {
            ...
            for (uint32_t mipLevel = 0; mipLevel < vk_texture_header->mipLevels; ++mipLevel)
            {
                ...
                if (_IsFormatCompressed(vk_texture_header->format))
                {
                    ...
                    texel_block_size = _GetCompressedFormatBlockSizeInBytes(vk_texture_header->format);
                }
                else if (_IsFormatRGBA(vk_texture_header->format))
                {
                    ...
                    texel_block_size = _GetRGBAFormatPixelBytes(vk_texture_header->format);
                }
                else
                {
                    ...
                    texel_block_size = _GetDepthStencilFormatPixelBytes(vk_texture_header->format, aspectIndex);
                }

                size_t stagingOffset_new = roundUp(roundUp(roundUp(roundUp(stagingOffset, static_cast<size_t>(4U)), static_cast<size_t>(texel_block_size)), optimalBufferCopyOffsetAlignment), optimalBufferCopyRowPitchAlignment);
